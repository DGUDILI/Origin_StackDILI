...
from app.schemas.predict import ToxicReason as ToxicReasonSchema
from app.services.chemistry import (
    get_physicochemical_props,
    smiles_to_svg_plain,
    validate_and_canonicalize,
)
from app.services.xai import MaccsPatternScore, ToxicReasonScore, abuild_xai_outputs
...
def _run_model_inference(
    ...
    canonical: str,
    include_xai: bool,
) -> tuple[float, list[MaccsPatternScore], list[ToxicReasonScore], str]:
    """
    ...
    Returns:
        (probability_pct, top_maccs, toxic_reasons, molecule_svg)
    ...
    toxic_reasons: list[ToxicReasonScore] = []
    svg = ""

    if include_xai:
        try:
            # probability_pct를 함께 전달 — SVG 색상(빨강/파랑) 결정에 사용
            top_maccs, toxic_reasons, svg = _run_xai_sync(mh_raw, canonical, n_atoms, probability_pct)
        except Exception as exc:
            logger.warning(
    ...
    return probability_pct, top_maccs, toxic_reasons, svg
...
def _run_xai_sync(
    ...
    return build_xai_outputs(mh_raw, canonical, n_atoms, prob, top_k=5)
...
async def predict_single(
    ...
    try:
        probability_pct, top_maccs, toxic_reasons, svg = await asyncio.to_thread(
