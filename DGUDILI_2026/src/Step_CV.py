...
print("=" * 60)
print(f"Step CV: DGUDILI 10-Fold CV  ({'CLEAN' if _USE_CLEAN else 'ORIGINAL'} data, pooling={POOLING})")
print("=" * 60)
...
for fold, (train_idx, test_idx) in enumerate(skf.split(X_fp_all, y_all), 1):
    ...
    # CrossAttentionEncoder (새 구조: chem_proj 내장, full 768-dim 입력)
    random.seed(SEED)
    np.random.seed(SEED)
    torch.manual_seed(SEED)
    encoder   = CrossAttentionEncoder(k=K, d_k=D_K, d_v=D_V).to(device)
    criterion = nn.BCEWithLogitsLoss()
    optimizer = torch.optim.AdamW(encoder.parameters(), lr=LR_RATE,
    ...
    print(f"  AUC={m['AUC']:.4f}  MCC={m['MCC']:.4f}  F1={m['F1']:.4f}  (best val AUC={best_val_auc:.4f})")
