...
if __name__ == "__main__":
    # 빠른 shape 검증
    B, N_atoms, N_kv, D = 2, 15, 167, 64
    model = DifferentialCrossAttention(d_model=D, num_heads=4, dropout=0.0)

    query    = torch.randn(B, N_atoms, D)
    key_val  = torch.randn(B, N_kv, D)

    # kv_padding_mask 없이 기본 동작 확인
    out, attn_w = model(query, key_val)
    print(f"output shape    : {out.shape}")      # (2, 15, 64)
    print(f"attn_weights    : {attn_w.shape}")   # (2, 15, 167)
    print(f"lambda example  : {model._compute_lambda().squeeze()}")

    # kv_padding_mask 동작 확인 (처음 50 bit 비활성화)
    kv_mask = torch.zeros(B, N_kv, dtype=torch.bool)
    kv_mask[:, :50] = True
    out2, attn_w2 = model(query, key_val, kv_padding_mask=kv_mask)
    inactive_attn = attn_w2[:, :, :50].abs().mean().item()
    active_attn   = attn_w2[:, :, 50:].abs().mean().item()
    print(f"inactive attn (should~0): {inactive_attn:.6f}")
    print(f"active attn             : {active_attn:.6f}")
    print("DifferentialCrossAttention shape 검증 완료")
